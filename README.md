...
| 층 | 교육 영역 | 주요 구성 |
|---|---|---|
| 1F 시설·보안층 | 전력, 냉방, 소방, 물리보안 | UPS·PDU·CRAC 등 가상 설비, 출입·CCTV 상태 |
| 2F 일반 전산실 | 시스템, 스토리지, 네트워크, 보안 | FW·IPS·WAF, 업무 앱, 서버·디스크 조사 |
| 3F AI 전산실 | GPU 플랫폼, 추론 서비스, 모델 운영 | GPU 존, WireGuard 게이트웨이, 모델 평가·배포 실습 |
| 4F 운영층 | NOC·SOC, 업무 조정, 검토·감사 | 관제, 업무 요청, 근무자 운영, AI 에이전트 관제 |
...
현재 [roster.yaml](agents/roster.yaml)은 **근무자 10명**, 연결된 업무 루프는 **12개**입니다. 시설, 물리보안, 네트워크, 시스템/스토리지, GPU, SOC, 서비스데스크, 운영 총괄, 감사, 애플리케이션 개발 역할을 나눕니다.
...
평시 감시 루프 5개는 **10분 → 5분 → 2분 → 1분**으로 점검 간격을 조정하고, 안정되면 점차 복귀합니다. 변화 없는 코드 점검에는 모델을 호출하지 않습니다. 별도 정기 보고·사용자 요청·사건 대응은 구독 사용량을 소비합니다.
...
| **근무자 운영** | `http://<관리 IP>:8050/` | 회사·부서·팀·근무자·루프·권한 |
...
4. **확인:** 답변의 조회 기록과 4층 관제에서 실제 도구 결과·근거·누락 사항을 확인합니다.
5. **개선:** 지침 실습에서 업무 방법을 수정하고 같은 조건의 새 대화·요청으로 비교합니다.
...
- AI 에이전트 관제의 읽기 API와 도구는 제공하지만, 관제 AI의 상시 자동 운영은 활성화하지 않았습니다.
