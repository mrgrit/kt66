...
| 뷰 | 내용 |
|---|---|
| **건물 뷰** | 4개 층을 **대각으로 엇갈려** 쌓았다(exploded axonometric) — 수직으로만 쌓으면 층 간격이 층 깊이보다 커야 해서 건물이 세로로만 길어지고, 전체를 보려면 축소하는 수밖에 없다. 랙의 LED 는 자산 하나가 한 줄 — **색은 존, 밝기는 실측 사용률**. 층 슬래브는 아일 온도로 달아오른다. 정지한 자산은 빨갛게 깜빡인다 |
