...
| 5F AI 연구소 | 전문가 방법론 연구·스킬 개발·업무 방식 실험실 | SOC 합성 사례, 사례별 판단·집계·안전·실측 토큰 비교 |
...
에이전트 관제 스키마는 요청·작업·실행·도구 호출을 연결하고, 승인 방식·파일 접근·설정/스킬 버전·토큰·검증 근거를 검색 가능한 필드로 보존합니다. [Wazuh 에이전트 관제 매뉴얼](docs/WAZUH-AGENT-MONITORING.ko.md)에 설치·컬럼 설명·사건 추적·검색 실습과 [가져오기용 대시보드](docs/wazuh/kt66-agent-observatory.ndjson)를 제공합니다.
...
5층의 **전문가 업무 방식 실험실**에서는 학생이 한국어 스킬을 수정하고 동일한 입력·모델로
기존 방법과 후보를 비교합니다. 신규 IP 집계·위험 우선순위·해외 접근 예외 등 SOC 사례가
기본 제공되며 평가 사례를 복제·편집할 수 있습니다. 사례별 판단·근거·수치와 실제 토큰을
함께 확인하고, 강사 검토 후 적용·복구합니다. [시작하기와 평가 사례 작성법](docs/EXPERT-WORKFLOW-LAB.ko.md)
...
| **AI 연구소** | `http://<관리 IP>:8050/research-lab` | 전문가 업무 방식 실험·평가 사례 편집·결과 비교·적용/복구 |
...
| [Wazuh 에이전트 관제](docs/WAZUH-AGENT-MONITORING.ko.md) | 전용 필드·대시보드 가져오기·권한/승인 조사·스킬별 결과 비교 |
