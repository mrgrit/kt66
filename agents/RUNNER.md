...
기존 11개 루프의 cadence와 triggers를 읽습니다. 사건의 정적 소유권은 루프 설정으로 정하고, 미배정 관측은 execution.dispatcher의 근무자에게 넘깁니다. 그 근무자는 생성 하네스를 바탕으로 업무를 배정합니다. 시나리오별 정답 조치 enum과 특정 보안 시험 전용 실행은 사용하지 않습니다.
...
현재 원본 주기는 화요일 기준 정기 회차 877회/일이며 사건/업무 전달은 별도입니다. 이는 구독이 그만큼 처리할 수 있다는 보장이 아닙니다.
...
- 54개 회귀 테스트 통과. 원본 사용자 수정은 보존했으며 커밋/푸시는 하지 않음.
