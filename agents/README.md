...
소견을 읽고 시작하므로, 되풀이 대신 **더할 것과 동의하지 않는 지점**을 적는다. L2 담당자의 소견이 있거나 소견이
...
- **활성 경보 전부를 하나의 사건으로 본다.** 이 랩의 고장은 대개 하나에서 연쇄하므로
  ...
  동시에 주입하면 한 티켓에 섞인다.
- **주기 분석은 명시적으로 켜야 한다.** `cc-runner --watch --scheduled`가 cadence를
  읽는다. 기본 실행은 경보 분석이며, 별도 서비스 등록 없이 백그라운드에서 돌지 않는다.
  주기 분석은 Claude·Bastion·Hermes 근무자를 지원한다. 자세한 범위와 예산은 [RUNNER.md](RUNNER.md)를 참조한다.
...
[RUNNER.md](RUNNER.md)에 실행 명령, 예산, 상태 조회와 미구현 연결을 정리했다.
렌더는 구성 파일 생성이며, 프로세스 시작이나 업무 성공을 의미하지 않는다.
