...
답변의 조회 기록과 왼쪽 메뉴의 **에이전트 실행 증적**을 함께 봅니다. 요청·트리거 → 상황·계획 → 도구 입력·결과 → 검토·결과 순서로 읽습니다. 도구 실패, 조회 범위 잘림, 미수집 자료도 확인합니다. **리스크·권한 관제** 탭에서는 해당 기록의 구역 이동·승인 대기·접근 거절을 재생할 수 있습니다.
...
에이전트 실행 증적과 업무 결과에서 입력·스킬 읽기·도구 호출·계획/검토 요약·산출물을
...
5층 **[전문가 업무 방식 실험실](EXPERT-WORKFLOW-LAB.ko.md)**에서 **SOC 예제로 시작**을
누르면 입력 자료와 후보 스킬이 준비됩니다. 한 가지 판단 기준을 한국어로 수정하고 개선 가설을
적으세요. 평가 후 사례별 기존/후보의 IP 선정·집계값·근거와 토큰을 비교하고 차이를 설명합니다.
실습 자료와 정답은 평가 사례 라이브러리에서 확인하며, 구체적인 정답을 스킬에 암기시키지 않습니다.
평가는 요청할 때만 모델을 2회 호출하고, 화면 조회·편집은 모델을 호출하지 않습니다.
