...
[프로젝트 안내](../README.md) · [교수·강사용 설치·수업 운영](INSTRUCTOR-GUIDE.ko.md) · [학생용 사용·지침 실습](STUDENT-GUIDE.ko.md)

대상은 KT66 운영자와 강사다. 최초 운영 검증은 **2026-09-15**, `b5d6ec2` 기준이며,
이후 직무 분리·사용자 업무·SOC 예약·스킬 로딩 변경을 반영했다. 과거 검증 기록은 해당 시점의 범위로 읽는다.
서버 예시는 `ccc@192.168.12.100`, 저장소는 `/home/ccc/work/kt66`를 기준으로 한다.
다른 서버에서는 주소와 계정·저장소 경로를 바꾼다. 아래 명령은 별도 표시가 없으면 이 서버에서 실행한다.

현재 근무자는 **10명**, 연결된 평시 루프는 **12개**다. 각 회차는 로그인된 Claude Code 또는 Codex CLI의 새 세션으로 실행된다.
...
| siem-alert-triage | soc-analyst | 코드 점검 10→5→2→1분 / 변화 시 AI | 새 특이사항에 한정한 사건 조사 | 없음 |
| soc-daily-risk-review | soc-analyst | 매일 09·18시 / `0 9,18 * * *` | IP별 종합 위험 분석·보고, 미산정 자료 명시 | 없음 |
...
| 4 근무자·R&R | R&R·스킬·담당 설정 / + 근무자 추가 / persona·roster 원문 | 자리·모델·역할·스킬·자산·루프 연결 |
| 5 일하는 방식 | 스킬 라이브러리 / 루프 / harness.yaml 직접 편집 | 스킬 추가·수정·삭제·연결, 회차와 정책 |
...
런타임·모델·직무·자율성·팀 선택은 변경 시 저장된다. 런타임을 바꾸면 해당 구독 경로에 맞는 모델을 함께 저장하며, 모델 목록도 호환되는 항목만 표시한다. 잘못된 런타임·모델 조합은 원본 저장 전에 거부한다.

**R&R·스킬·담당 설정**에서 역할 요약·본문과 스킬·루프·담당 자산을 관리한다. 부서 책임과 실제 컴파일러의 상속 정책도 확인할 수 있다. **페르소나 원문**과 **roster.yaml 전체 원문 편집**도 사용할 수 있다.

스킬 추가·수정·삭제는 5단계 라이브러리에서 수행한다. 상세 경로와 예시는 [근무자 설정·R&R·스킬 관리](AGENT-CONFIGURATION.ko.md)를 참고한다.
...
조직·persona·루프·스킬·담당 설정은 저장 전에 임시 사본에서 컴파일한다. 저장 후에는 생성 포인터와 활성 버전을 갱신한다. 저장 뒤 파일 시스템 오류 등으로 적용에 실패하면 성공으로 간주하지 않고 오류 메시지와 적용 상태를 확인한다.

**하네스 다시 생성**은 원본을 검증하고 전체 하네스와 `activation.json`을 함께 갱신한다. **화면·원본·적용 상태 대조**에서 연결 오류와 버전 일치를 확인하고, 실제 다음 회차의 `loaded-harness.json` 및 도구 기록으로 사용 여부를 확인한다. 백업 복원도 원래 경로에 검증·재적용한다.
...
루프 저장 API는 파일명·id·owner·steps, 실행기가 사용하는 cron 문법과 명단 연결을 검사한다. 자연어 단계의 업무 의미까지 검증하지 않는다.
...
별도 일정의 방화벽·GPU 쿼터·백업·출입·전력 점검 13회와 SOC 종합 보고 2회로 합계 15회/일이고 월요일은 주간 감사가 추가된다. 사건·전달·승인·사후검증·재시도와 발견된 변경에 따른 조사는 추가될 수 있다. `max_tokens`는 여전히 CLI 토큰 강제 중단 한도가 아니므로, 집계 토큰을 구독 잔량이나 보장된 소비 상한으로 읽지 않는다.
